...
**Test credentials received from AT** (asi-cd@at.gov.pt, following the templates in the two "Aspetos Genéricos" manuals above): `TesteWebservices.pfx` (client cert for mutual TLS) and `Chave Cifra Publica AT 2027.cer`/`.p7b` (AT's public key for encrypting the SOAP-header password). Verified by byte inspection, not just trusted: cert subjects/validity match what the manuals describe. **Not committed** — `.gitignore` covers `*.pfx`/`*.p12`/`*.cer`/`*.p7b`. Kept locally at `api/var/at-webservices/` (task 3.1: a repo-root `docker/keys/` location was tried first and didn't work, since `docker-compose.yml` never mounts anything outside `api/` into the `php` container). Confirmed reusable across both the e-Fatura and series webservices (`at-ws-series-aspetos-genericos.pdf` §3, quoted above) — one request covered both.
