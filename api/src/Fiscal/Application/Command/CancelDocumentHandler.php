 ...
 * 8632/2014 §3.3.8) and not (plausibly) yet delivered to the customer,
 * approximated here by "not yet communicated to the AT" (CIVA Art. 29.º
 * §7). Writes the `document_status_events` row task 2.3's own trigger
 * requires before allowing the `documents.status` change, via the same
 * {@see DocumentWriter::updateStatus()} task 2.8's conversion-closing
 * already added. Stock/account compensating entries would reverse this
 * cancellation's effects here, same as task 2.6 leaves issuance's own
 * side effects unwired — neither stock movements (Phase 5) nor current
 * accounts (§6.7) exist yet, so there is nothing to compensate yet.
 */
